...
export default function Simulation() {
  ...
  const runSimulation = async () => {
    ...
    try {
      setLoading(true);
      const response = await predictFraud(parsed, model);
      setResult(response);
    } catch (apiError) {
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <div className="rounded-xl border bg-white p-6">
          ...
          <div className="mb-4">
            ...
            >
              <option value="stack">Stacking (Final System)</option>
          ...
          <div className="mt-4 flex items-center gap-3">
            ...
            <button
              ...
              onClick={() =>
                setPayloadText(prettyJson(parsedPreview || DEFAULT_PAYLOAD))
              }
            >
              Format JSON
            </button>
          ...
          <div className="rounded-xl border bg-white p-6">
            ...
            <div className="grid grid-cols-1 gap-2 text-sm text-slate-600 sm:grid-cols-2">
              {REQUIRED_FIELDS.map((field) => (
                <div
                  key={field}
                  className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 font-mono text-xs"
                >
                  {field}
                </div>
              ))}
